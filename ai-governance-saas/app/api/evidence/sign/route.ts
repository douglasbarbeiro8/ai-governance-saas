...
import { z } from "zod";
const Body = z.object({ assessmentId: z.string().uuid(), questionId: z.number().int().positive(), filename: z.string().min(1), mime: z.string().min(3) });
const BUCKET = "evidence";
async function getAssessmentOrg(supabase: any, assessmentId: string) {
  const { data, error } = await supabase.from("assessments").select("org_id").eq("id", assessmentId).single();
  if (error) throw error; return data.org_id as string;
}
...
export async function POST(req: NextRequest) {
  ...
  const client = createServerClient(process.env.NEXT_PUBLIC_SUPABASE_URL!, process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!, { cookies: { get: (key) => cookieStore.get(key)?.value } });
